  ...
  <g>
    {#if phase > 0}
      <text
        x="0"
        y="0"
        transform="translate({hsize} {hsize *
          1.25}) scale({textScale},{textScale})"
        style="text-anchor: middle">{text}</text
      >
    {/if}
    <path
      id="arrow"
    ...
    >
      <g>
        <use href="#arrow" fill={phase1FillCol} stroke={phase1Col}>
          {#if phase == 1}
            <animate
              attributeName="fill"
              values="white;{phase1Col};white"
              dur="3s"
